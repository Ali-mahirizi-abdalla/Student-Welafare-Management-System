...
class StaffProfile(models.Model):
    ROLE_CHOICES = [
        ...
        ('HEALTH_ADMIN', 'Health Services Admin'),
        ...
        ('ACC_IN_CHARGE', 'Accommodation Officer'),
    ...
    def get_category(self):
        ...
        if role in ['Super Admin']:
            return 'EXECUTIVE'
        if role in ['TVET_DIRECTOR', 'DEAN_STUDENTS']:
            return 'ACADEMIC_ADMIN'
        if role in ['FINANCE_OFFICER', 'AUDIT_LOGS']:
            return 'FINANCE_ADMIN'
        if role in ['SPORTS_OFFICER', 'NEWS_ALERT', 'ACC_IN_CHARGE']:
            return 'STUDENT_SERVICES'
        if role in ['ICT_MANAGER', 'MAINTENANCE_HEAD', 'VISITORS']:
            return 'TECHNICAL_ESTATES'
        if role in ['CAMPUS_NURSE', 'CAMPUS_COUNSELOR', 'HEALTH_ADMIN']:
